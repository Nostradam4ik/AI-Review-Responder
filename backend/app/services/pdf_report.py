...
def generate_pdf_bytes(analysis: dict, meta: dict) -> bytes:
    ...
    """
    business_name = meta.get("business_name", "Your Business")
    location_name = meta.get("location_name", "") or "All Locations"
    period_label = meta.get("period_label", "")
    total_reviews = meta.get("total_reviews", 0)
    response_rate = meta.get("response_rate", 0)
    generated_at = meta.get("generated_at", datetime.now().strftime("%B %d, %Y"))
    ...
    footer_text = f"{business_name} · {period_label} · Confidential · Generated by AI Review Responder"
